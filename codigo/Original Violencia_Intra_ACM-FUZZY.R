...
datos_1$`Contexto del Hecho` <- dplyr::recode(
  datos_1$`Contexto del Hecho`,
  ...
  .default = datos_1$`Contexto del Hecho`
)


names(datos_1) <- gsub("Circunstancia.del.Hecho.Detallada",
...
# Con la diferencia que la variable Contexto del Hecho entra como suplementaria
 ...
 df_clean0 <- datos_1[, c(cat_vars, "id")]
 ids <- df_clean0$id
 df_clean <- df_clean0 %>% select(-id)
 ...
 coord <- ACM_datos$ind$coord
 
 # Crear data frame con las primeras 3 dimensiones
 df_plot <- data.frame(
   Dim1 = coord[,1],
   Dim2 = coord[,2],
   Dim3 = coord[,3]
 ...
 plot_ly(df_plot,
         ...
         mode = "markers",
         marker = list(size = 3)) %>%
   layout(title = "ACM - Dimensiones 1, 2 y 3")
...
# coord_activos <- ACM_datos$ind$coord[, 1:4]
 ...
 #ids_activos <- ids[-ind_sup]
 #ids_suplem  <- ids[ind_sup]
 
 #ids_totales <- c(ids_activos, ids_suplem)
 ...
 ids_totales <- c(
   rownames(ACM_datos$ind$coord),
   rownames(ACM_datos$ind.sup$coord)
 )
 ...
 datos_1$id <- as.character(datos_1$id)
 ...
 df_clusters <- data.frame(
   id = ids_totales,
   Cluster_Fuzzy = cluster_asignado
 )
df_clusters$id <- as.character(df_clusters$id)
 df_final <- datos_1 %>%
 ...
 df_final$Cluster_Fuzzy <- as.factor(df_final$Cluster_Fuzzy)
 
 table(df_final$Cluster_Fuzzy2, useNA = "always")
 table(df_final$Cluster_Fuzzy, useNA = "always")
 ...
 prop.table(table(df_final$Cluster_Fuzzy, df_final$Contexto.del.Hecho), 1) * 100
 
 # Comparar porcentajes de Sexo por Cluster
 prop.table(table(df_final$Cluster_Fuzzy, df_final$Sexo.de.la.victima), 1) * 100
 
 
 # Comparar porcentajes de ciclo vital por Cluster
 prop.table(table(df_final$Cluster_Fuzzy, df_final$Ciclo.Vital), 1) * 100
 # Comparar porcentajes de circunstancia del hecho por Cluster
 prop.table(table(df_final$Cluster_Fuzzy, df_final$Circunstancia.del.Hecho), 1) * 100
 # Comparar porcentajes de presunto agresor por Cluster
 prop.table(table(df_final$Cluster_Fuzzy, df_final$Presunto.Agresor.Detallado), 1) * 100
 ...
 # Opcional: Exportar a Excel de los perfiles
  ...
  browseURL("--Perfil_Clusters_Fuzzy.xlsx")
  ...
  df_final$pertenencia_max <- apply(matriz_u, 1, max)
  
  #estadísticas descriptivas de la certeza
  summary(df_final$pertenencia_max)
  
  
  # Se crean las etiquetas de seguridad en la asignación
  df_final$tipo_pertenencia <- cut(df_final$pertenencia_max, 
  ...
  tabla_ambiguedad <- table(df_final$tipo_pertenencia)
  ...
  df_final$gap_pertenencia <- p1 - p2
  
  
  casos_puente_pro <- df_final[df_final$gap_pertenencia < 0.1, ]
  ...
  df_final$entropia <- apply(matriz_u, 1, calcular_entropia)
  
  # Normalizar la entropía (de 0 a 1, donde 1 es máxima confusión)
  # Para 4 clusters, el log2(4) es 2.
  df_final$entropia_norm <- df_final$entropia / log2(4)
  
  summary(df_final$entropia_norm)
  
  
  colnames(matriz_u) <- paste0("Cluster_", 1:ncol(matriz_u))
  
  df_final$top_cluster1 <- apply(matriz_u, 1, function(x) colnames(matriz_u)[order(x, decreasing = TRUE)[1]])
  df_final$top_cluster2 <- apply(matriz_u, 1, function(x) colnames(matriz_u)[order(x, decreasing = TRUE)[2]])
  # Crear una matriz de "Confusión Interna" para los casos ambiguos (Gap < 0.15)
  tabla_solapamiento <- df_final %>%
  ...
  # Grafico de violin 
  ggplot(df_final, aes(x = as.factor(Cluster_Fuzzy), y = entropia_norm, fill = as.factor(Cluster_Fuzzy))) +
  ...
  todos_3d <- data.frame(
    id = ids_totales,
    Dim1 = coord_totales[,1],
    Dim2 = coord_totales[,2],
    Dim3 = coord_totales[,3]
  )
  ...
  todos_3d$Cluster <- factor(
    todos_3d$Cluster_Fuzzy,
    levels = c(1,2,3,4),
    labels = c(
      "C1: Violencia de Pareja (Actual y Expareja)",
      "C2: Violencia relacional con consumo de sustancias",
  ...
  fig <- plot_ly(
    ...
    # SOLO puntos del plano (pequeños)
    marker = list(size = 2, opacity = 0.4)
  ) %>%
    
   
    layout(
      title = "Espacio Factorial 3D: Perfiles de Violencia",
  ...
  colores_clusters <- c(
    "C1: Violencia de Pareja (Actual y Expareja)" = "#D55E00",
    "C2: Violencia relacional con consumo de sustancias" = "#0072B2",
    "C3: Violencia contra NNA" = "#009E73",
    "C4: Violencia Mixta o transacional" = "#CC79A7"
  )
