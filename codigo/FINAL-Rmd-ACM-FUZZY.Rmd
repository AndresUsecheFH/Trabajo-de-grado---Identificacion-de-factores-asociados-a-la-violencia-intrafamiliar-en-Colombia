---
title: "Fisher--ACM's--FUZZY-C-MEANS"
...
# FISHER
 ...
 Algunas asociaciones importantes no pueden evaluarse mediante la prueba Chi-cuadrado de independencia, debido al incumplimiento de sus supuestos (por ejemplo, bajas frecuencias esperadas).En estos casos, se utiliza la prueba exacta de Fisher como alternativa.
...
Dado el tamaño del conjunto de datos y la dimensionalidad de las tablas de contingencia, el cálculo exacto de la prueba de Fisher no es computacionalmente viable. En su lugar, se emplea una aproximación basada en simulación de Monte Carlo con 100000 replicaciones para estimar el valor p.
```{r}
...
Se genera una copia del conjunto de datos, se recodifica una de las variables categóricas para mejorar su interpretación y se distinguen las variables e individuos suplementarios,los cuales no participan en la construcción del espacio factorial del análisis pero si se pueden representar en los planos factoriales.
...
```
 ...
 Los registros que presentan más del 15% de valores faltantes o categorías no válidas son tratados como individuos suplementarios, por lo que no participan en la construcción del espacio factorial, pero se proyectan posteriormente para su interpretación.
...
Categorías tratadas como suplementarias: se incluyen aquellas con baja asociación estadística dentro del conjunto de variables, así como aquellas que presentan menos del 80% de información válida. Estas no participan en la construcción del espacio factorial, pero se proyectan para su interpretación.
...
```
Representación de las categorias en los ejes
...
```
### Representación en el plano factorial

Representación en el plano factorial de los individuos y las 12 categorias que mas contribuyen en las construciones de los ejes
```{r}
...
Análisis de la proyección de los individuos en las dos primeras dimensiones factoriales según el sexo de la víctima
```{r}
...
```
Análisis de la proyección de los individuos en las dos primeras dimensiones factoriales según el Contexto del Hecho
```{r}
...
Análisis de la proyección de los individuos en las dos primeras dimensiones factoriales según el ciclo vital
```{r}
...
```
 ...
 Si se quiere descargar los valores propios y demas información del ACM I ejecutar las siguientes instrucciones
...
Se mantienen los mismos pasos que se realizaron con el ACM I con la diferencia que la variable Contexto del Hecho entra como suplementaria
...
Contribuciones de las categorias a los ejes
```{r}
...
Categorias que mejor se representan en los ejes
```{r}
...
### Representación de los individuos y categorias en los planos factoriales
```{r}
...
```
Caracterizando por sexo de la victima
```{r}
...
Caracterizando por contexto del hecho
```{r}
...
```
Caracterizando por ciclo vital
```{r}
...
Caracterizando por circunstancias del hecho
```{r}
...
```
Solo si se quieren descargar los valores propios y demas valores
```{r}
...
# FUZZY C-MEANS
La determinación del número de clústeres se realiza mediante métodos tradicionales como el criterio del codo (WSS) y el índice de silueta, utilizando k-means como aproximación inicial.Posteriormente, se evalúa la calidad de la partición en el contexto de clustering difuso mediante métricas como el Partition Coefficient (PC) y la Partition Entropy (PE). Debido al alto costo computacional, estos procedimientos se aplican sobre una muestra del conjunto de datos.
```{r,eval=FALSE}
...
```
Se evalúan distintas configuraciones del número de clústeres (k = 3, 4 y 5) utilizando métricas  Partition Coefficient (PC) y la Partition Entropy (PE), con el fin de analizar la calidad de la partición.

Entre mayor sea el coeficiente de partición y entre menor sea la partición de entropia mejor, por lo tanto lo mejor es encontrar un buen punto de equilibrio entre las dos metricas.
...
Se decide agrupar con 4 clusters
...
```
El promedio de pertenencia de cada punto de pertenecer a un cluster es de 0.75, lo cual significa que en general cada punto esta bien definido por un cluster
...
```
El 67.20% de las asignaciones son claras, el  28.96% estan en un nivel intermedio y el 3.82% son muy difusas
...
VISUALIZACIÓN DE LOS CLUSTERS SOBRE EL PLANO FACTORIAL DEL ACM 
DIMENIONES 2 Y 3
